...
namespace Cardiology.Data.PostgreSQL
{
    public class PgDdtAnamnesisService : IDdtAnamnesisService
    {
        ...
        public DdtAnamnesis GetByHospitalSessionId(string id)
        {
            using (dynamic connection = connectionFactory.GetConnection())
            {
                String sql = String.Format("SELECT r_object_id, dss_anamnesis_vitae, dss_drugs_intoxication, dss_cardiovascular_system, dsdt_inspection_date, dss_past_surgeries, dss_diagnosis_justification, dsb_template, dss_operation_cause, dss_anamnesis_epid, dss_urinary_system, dss_anamnesis_allergy, dss_digestive_system, dss_nervous_system, dss_diagnosis, r_creation_date, dss_complaints, dss_oks_st, dsid_doctor, dsid_patient, dsid_hospitality_session, dss_respiratory_system, r_modify_date, dss_st_presens, dss_template_name, dss_accompanying_illnesses, dss_anamnesis_morbi FROM ddt_anamnesis WHERE dsid_hospitality_session = '{0}'", id);
                Npgsql.NpgsqlCommand command = new Npgsql.NpgsqlCommand(sql, connection);
                using (DbDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
        ...
        public DdtAnamnesis GetByTemplateName(string templateName)
        {
            using (dynamic connection = connectionFactory.GetConnection())
            {
                String sql = String.Format("SELECT r_object_id, dss_anamnesis_vitae, dss_drugs_intoxication, dss_cardiovascular_system, dsdt_inspection_date, dss_past_surgeries, dss_diagnosis_justification, dsb_template, dss_operation_cause, dss_anamnesis_epid, dss_urinary_system, dss_anamnesis_allergy, dss_digestive_system, dss_nervous_system, dss_diagnosis, r_creation_date, dss_complaints, dss_oks_st, dsid_doctor, dsid_patient, dsid_hospitality_session, dss_respiratory_system, r_modify_date, dss_st_presens, dss_template_name, dss_accompanying_illnesses, dss_anamnesis_morbi FROM ddt_anamnesis WHERE dss_template_name = '{0}'", templateName);
                Npgsql.NpgsqlCommand command = new Npgsql.NpgsqlCommand(sql, connection);
                using (DbDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
